...
```text
notebooks/06_colab_training_evaluation.ipynb
```
...
```bash
cd ..
zip -r fraud-detection.zip fraud-detection \
  -x "fraud-detection/data/raw/*" \
  -x "fraud-detection/data/warehouse/*" \
  -x "fraud-detection/.git/*" \
  -x "fraud-detection/__pycache__/*" \
  -x "fraud-detection/*/__pycache__/*"
```
...
```text
docs/model_accuracy_diagnosis.md
```
