...
1. Prétraitement et découpage
   ...
   - Fichiers modifiés: [src/preprocessing.py](src/preprocessing.py), notebooks: [notebooks/02_preprocessing.ipynb](notebooks/02_preprocessing.ipynb).
...
2. Pipeline d'entraînement
   ...
   - Fichiers modifiés: [src/train.py](src/train.py). Fonctions clés: `run_training_pipeline()`, `_build_models()`, `_evaluate_estimator()`.
...
3. Enregistrement des artefacts
   ...
   - Fichiers utilisés: [src/train.py](src/train.py) (sauvegarde), [src/evaluate.py](src/evaluate.py).
